...
def create_dictionary(directory):
    ...
    for path in Path(directory).glob("*"):
        if path.is_file():
            ...
            file_name = path.name
            files_dict.update({file_name: file_hash})
...
def get_timestamp(file, directory):

    filepath = Path(directory) / file

    if filepath.exists():
        unix_time = filepath.stat().st_mtime
...
def write_change(file, action, directory):
...
def monitor(directory):
	...
	while True:
		...
		try:
			...
			changes = check_changes(directory)
			...
			if changes:
				...
				for action in changes:
					...
					for file_changed in action:
						timestamp = get_timestamp(file_changed, directory)
	                       	
						if action_name != "Deleted":
								file_hash = calc_hash(Path(directory) / file_changed)
						else:
						...
						if not already_prompted(file_changed, action_name, file_hash):
							...
							log_change(action_name, file_changed, confirmed, timestamp, directory)

						else:
							confirmed = False
	
						if confirmed:
							write_change(file_changed, action_name, directory)
...
def check_changes(directory):
    ...
    curr_dictionary = create_dictionary(directory)
...
def log_change(action_performed, file_changed, confirmed, timestamp, directory):
...
def main():
    ...
    if args.path:
        set_directory = Path(args.path).resolve()
    
        if set_directory.exists() and set_directory.is_dir():
            directory = str(set_directory)
        else:
            print("the set directory doesnt exist or is a file.")
            return
    else:
        try:
            directory = Path("files").resolve()
        except FileNotFoundError:
            print("Default monitor directory not found")
            return
...
parser.add_argument("--path", help="Sets monitored directory.")
